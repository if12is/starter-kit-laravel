    ...
    <form id="form-modalAction" class="form" action="{{ route('permissions.store') }}" method="POST">
        ...
        @if (count($permissions) > 0)
            <div class="col-md-3 mb-3">
                <input type="text" id="searchPermission" class="form-control" placeholder="ابحث عن الصلاحية">
            </div>
            <div class="row">
                <div class="col-md-4">
                    <div class="form-check">
                        <input class="form-check-input" type="checkbox" id="checkAll">
                        <label class="form-check-label fw-bolder" for="checkAll">تحديد الكل</label>
                    </div>
                </div>
            </div>
        @endif
